...
(defun get-day-of-week (month day year)
  (let* ((yr100 (mod year 100)) 
    ...
    (if (and (zerop (rem year 4)) (not (= yr100 0)) (or (string-equal month "Jan") (string-equal month "Feb")) ) (setf lp "_l"))
    ;(format t "eval: ~A~%"  `(concatenate 'string ,month ,lp))
    (setf key (eval `(cdr (assoc (concatenate 'string ,month ,lp) *mokey* :test #'string-equal))))
    ;(format t "yr100: ~A yr-rem: ~A lp: ~A key: ~A ~%" yr100 yr-rem lp key )
    (cond
    ...
    ;(format t "yr-rem: ~A key: ~A ~%" yr-rem key)
    (if yr-rem 
